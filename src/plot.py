...
plt.viewModel(f"fwi_vp_acoustic_Nx201_Nz201.bin")
...
# plt.viewSeismogram(f"{pmt.seismogramFolder}seismogram_obs_shot_4_Nt{pmt.nt}_Nrec{pmt.Nrec}.bin", perc=99)
...
plt.viewImage(f"../outputs/migrated_image/gradient_fwi_iter_1_acoustic_Nx201_Nz201.bin",laplacian=False,perc=99)
...
model_smooth = np.fromfile("../inputs/diffractorvp_Nz201_Nx201.bin", dtype=np.float32).reshape(pmt.nz,pmt.nx)
model = np.fromfile("../inputs/fwi_vp_acoustic_Nx201_Nz201.bin", dtype=np.float32).reshape(pmt.nz,pmt.nx)
